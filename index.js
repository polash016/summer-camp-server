...
async function run() {
  try {
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          const result = await classCollection
            .find({ instructor_email: { $in: instructorEmails } })
            .toArray();
          res.send(result);
        } catch (error) {
    ...
    app.post("/payments", verifyJWT, async (req, res) => {
      ...
      const query = { _id: new ObjectId(payment.selectedCourse) };
      ...
      console.log(query, classQuery, classUpdate)
      const updatedResult = await classCollection.updateOne(
        classQuery,
        classUpdate
      );
      res.send({ insertResult, deletedResult, updatedResult });
    });
  } finally {
